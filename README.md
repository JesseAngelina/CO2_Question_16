...
The classes are different, but the same processing function can work with both objects.
...
For example, instead of checking the class using isinstance(), we can directly call:

source.read()

If the object supports read(), the function works correctly.

Therefore, explicit type checking is generally not necessary when the required method is available.
